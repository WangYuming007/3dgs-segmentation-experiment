...
def octree_ball_query(coords, mean_active, radius):
    ...
    _C.build_and_export_octree(coords, xyzwhl, boxes, pt_inds, pt_start_len, num_levels)
    boxes = boxes.cuda()
    ...
    while True:
        ...
        n_totals = _C.octree_ball_query(coords, boxes, pt_inds, pt_start_len, out_inds,
...
class GetMaskIoUOnCluster(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum):
        ...
        _C.get_mask_iou_on_cluster(proposals_idx, proposals_offset, instance_labels,
...
class GetMaskIoUOnPred(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum,
        ...
        _C.get_mask_iou_on_pred(proposals_idx, proposals_offset, instance_labels,
                                 instance_pointnum, proposals_iou, nInstance, nProposal,
                                 mask_scores_sigmoid)
...
class GetMaskLabel(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_cls,
        ...
        _C.get_mask_label(proposals_idx, proposals_offset, instance_labels, instance_cls,
...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        _C.voxelize_idx(coords, output_coords, input_map, output_map, batchsize, mode)
...
class Voxelization(Function):
    ...
    def forward(ctx, feats, map_rule, mode=4):
        ...
        _C.voxelize_fp(feats, output_feats, map_rule, mode, M, maxActive, C)
        return output_feats
    ...
    def backward(ctx, d_output_feats):
        ...
        _C.voxelize_bp(d_output_feats.contiguous(), d_feats, map_rule, mode, M, maxActive, C)
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        while True:
            ...
            nActive = _C.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n,
                                            meanActive, radius)
...
class BFSCluster(Function):
    ...
    def forward(ctx, cluster_numpoint_mean, ball_query_idxs, start_len, threshold, class_id):
        ...
        # print('start_len',start_len.shape)
        _C.bfs_cluster(cluster_numpoint_mean, ball_query_idxs, start_len, cluster_idxs,
...
class GlobalAvgPool(Function):
    ...
    def forward(ctx, feats, proposals_offset):
        ...
        _C.global_avg_pool_fp(feats, proposals_offset, output_feats, nProposal, C)
    ...
    def backward(ctx, d_output_feats):
        ...
        _C.global_avg_pool_bp(d_feats, proposals_offset, d_output_feats.contiguous(), nProposal, C)
...
class SecMean(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _C.sec_mean(inp, offsets, out, nProposal, C)
...
class SecMin(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _C.sec_min(inp, offsets, out, nProposal, C)
...
class SecMax(Function):
    ...
    def forward(ctx, inp, offsets):
        ...
        _C.sec_max(inp, offsets, out, nProposal, C)
