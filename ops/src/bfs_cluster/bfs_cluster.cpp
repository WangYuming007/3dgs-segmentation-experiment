...
void bfs_cluster(at::Tensor class_numpoint_mean_tensor,
  ...
  int num_elements = start_len_tensor.numel();
    // 打印验证

  // 创建一个新的内存空间，并将start_len_tensor的数据复制到这个内存中
  // int *start_len = new int[num_elements];  // 动态分配内存
  // std::memcpy(start_len, start_len_tensor.data_ptr<int>(), num_elements * sizeof(int));
